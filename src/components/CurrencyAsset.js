...
const CurrencyAsset = ({
  ...
  // 숫자로 변환하고, NaN일 경우 기본값 0을 설정
  const formattedExpectedProfit = isNaN(Number(expectedProfit)) ? 0.00 : Number(expectedProfit).toFixed(2);
  const formattedExpectedExchangeRate = Number(expectedExchangeRate).toFixed(2);
  ...
  return (
    <div className="bg-white w-full rounded-2xl">
      ...
      <div className="flex justify-between">
        ...
        <Text className="text-slate-600">{formattedExpectedExchangeRate}</Text>
      </div>

      {/* 예상 수익금 */}
      <div className="flex justify-between">
        <Text>예상수익금 ({currencyCode})</Text>
        <Text className='font-bold' color={Number(expectedProfit) >= 0 ? "red.500" : "blue.500"}>
          {formattedExpectedProfit}원
        </Text>
      </div>
      ...
      {/* <div className="flex justify-between">
        ...
        <Text className='font-bold' color={Number(expectedProfitRate) >= 0 ? "red.500" : "blue.500"}>
          {expectedProfitRate}%
        </Text>
      </div> */}
    </div>
