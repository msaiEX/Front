...
const MainPageDetail = () => {
  ...
  const { id } = useParams();
  // increase 값을 설정하는 함수
  const getIncreaseValue = (currency) => {
    switch (currency) {
      case "USD":
        return 0.04;
      case "JPY":
  ...
  const increase = getIncreaseValue(changeState);
    ...
    const getUseHistory = async () => {
      try {
        const result = await axios.get(
          `http://34.22.76.4:8081/api/mainpage/detail?state=${id}`
        );
        ...
        const filteredTodayData = result.data.todayData.filter(
          (item) => item.state === changeState && item.date === todayDate
        );
        ...
        if (filteredTodayData.length > 0) {
          ...
          // 현재 통화의 우대율 가져오기
          const rate = rates[changeState] || 0;
  ...
  const handlePurchaseClick = () => {
    ...
    if (
      ...
      // 데이터 전송
      axios
        .post("http://34.22.76.4:8082/api/trinsert", transactionData)
        .then((response) => {
      ...
      // 데이터 전송
      axios
        .post("http://34.22.76.4:8082/api/trinsert", reserveTransactionData)
        .then((response) => {
  ...
  const handleTransactionUpdate = (updatedTransaction) => {
    ...
    axios
      .put("http://34.22.76.4:8082/api/updateTransaction", transactionToUpdate)
      .then((response) => {
  ...
  const handleSelectClick = () => {
    ...
    // 서버에서 예약된 거래 내역 가져오기
    axios
      .get("http://34.22.76.4:8082/api/reserved")
      .then((response) => {
    ...
    <div className="w-full h-full relative flex justify-center">
      <div className="w-[960px] flex flex-col py-1 px-10">
        ...
        >
          <button
            onClick={onOpen}
            className="text-white bg-sky-500 px-4 py-2 rounded hover:bg-white hover:border hover:border-sky-500 hover:text-sky-500 transition-all"
            ...
            <div className="flex items-center mb-3">
              ...
              <Image
                ...
                src={
                  ...
                  `/image/${changeState.toLowerCase()}_flag.png`
                }
              ></Image>
            </div>
            <div>
            ...
            >
              ...
              <div className="mt-2">
                <Text className="mx-4 text-sm">
                  24년 들어, 인플레이션 지속적으로 둔화되는 가운데 고금리 장기화
              ...
              <Text className="text-slate-570 text-sm ml-2 mt-2">
                자료 : 하나은행, 하나금융경제연구소
              </Text>
            </div>
          </div>
