...
const MainPage = () => {
  ...
  useEffect(() => {
    const getUseHistory = async () => {
      try {
        const result = await axios.get("http://34.22.76.4:8081/api/mainpage");
        const result2 = await axios.get(
          "http://34.22.76.4:8081/api/mainpage/worldtable"
        );
          ...
          <div className="flex justify-center mt-3">
            {filteredData.length > 10 && (
              <button onClick={() => setShowAll(!showAll)}>
                {showAll ? (
                  <div className="w-[120px] flex justify-center items-center rounded-full p-1 bg-sky-500 text-white hover:bg-white hover:text-sky-500 border border-sky-500 transition duration-300">
                    접기 <TiArrowSortedUp />
                  </div>
                ) : (
                  <div className="w-[120px] flex justify-center items-center rounded-full p-1 bg-sky-500 text-white hover:bg-white hover:text-sky-500 border border-sky-500 transition duration-300">
