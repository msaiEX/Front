...
const AiModal = ({ onClose, aiData }) => {
  ...
  // weekPredict 데이터를 JSON 형태로 변환
  const weekPredictData = JSON.parse(aiData.weekPredict);
  // 차트에서 사용할 수 있도록 데이터 변환
  const formattedData = Object.keys(weekPredictData[0]).map((date) => ({
    date, // 날짜를 x축에 사용
    value: parseFloat(weekPredictData[date]), // 값을 y축에 사용
  }));
  console.log(formattedData)
  console.log(weekPredictData[0])
            ...
            <TabPanel>
              ...
              <div className="pt-12 pb-8 pl-8 pr-10 bg-gray-700 rounded-xl mt-5">
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart data={weekPredictData[0]}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="date" stroke="#f2edd3" />
                    <YAxis stroke="#f2edd3" domain={[1320, 1340]} />
                    <Tooltip />
                    <Area
                      type="monotone"
                      dataKey="value"
