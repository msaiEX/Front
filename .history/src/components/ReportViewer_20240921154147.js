    ...
    <div className="w-full px-5">
      ...
      <div className="flex justify-center bg-[#d6e1fd] my-4">
        <button
          ...
          className={`w-[25%] px-2 py-2 rounded ${activeButton === 'year' ? 'bg-blue-200 text-blue-700' : 'bg-[#f1f3f8] text-slate-700'}`}
        >
          Year Report
        </button>
        <button
          onClick={() => handleReportChange('month')}
          className={`w-[25%] px-2 py-2  ${activeButton === 'month' ? 'bg-blue-200 text-blue-700' : 'bg-[#f1f3f8] text-slate-700'}`}
        >
          Month Report
        </button>
        <button
          onClick={() => handleReportChange('week')}
          className={`w-[25%] px-2 py-2  ${activeButton === 'week' ? 'bg-blue-200 text-blue-700' : 'bg-[#f1f3f8] text-slate-700'}`}
        >
          Week Report
        </button>
        <button
          onClick={() => handleReportChange('day')}
          className={`w-[25%] px-2 py-2  ${activeButton === 'day' ? 'bg-blue-200 text-blue-700' : 'bg-[#f1f3f8] text-slate-700'}`}
        >
          Day Report
        </button>
      </div>
      ...
      <div className="flex mt-6 gap-2">
        <a
          ...
          download // 다운로드 속성 추가
          className="px-4 py-2 bg-blue-200 text-blue-600 rounded flex items-center"
        >
          반기상황
          <IoMdDownload />
        </a>
        <a
          ...
          download // 다운로드 속성 추가
          className="px-4 py-2 bg-blue-200 text-blue-600 rounded"
        >
          월간사황
        </a>
        <a
