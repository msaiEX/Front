...
const CurrencyAsset = ({
  ...
  return (
    <div className="bg-white w-full rounded-2xl">
      ...
      <div className="flex justify-between">
        <Text className="text-slate-600">예상적용환율 ({currencyCode})</Text>
        <Text className="text-slate-600">{expectedExchangeRate.toFixed(2)}</Text>
      </div>

      {/* 예상 수익금 */}
      <div className="flex justify-between">
        <Text>예상수익금 ({currencyCode})</Text>
        <Text className='font-bold' color={expectedProfit >= 0 ? "red.500" : "blue.500"}>
          {isNaN(expectedProfit) ? 0 : expectedProfit.toFixed(2)}원
        </Text>
      </div>
