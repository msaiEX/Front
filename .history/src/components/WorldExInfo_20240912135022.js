...
const WorldExInfo = ({ currency, baseRate, remitSend, remitReceive, updownRate, isSelected }) => {
  ...
  return (
    <div className="w-full text-center flex mb-2 py-2 rounded-full" style={{boxShadow: "0 3px 13px rgba(0, 0, 0, 0.2)"}}>
      <div className="w-[20%] flex gap-1 justify-start items-center px-6">
      ...
      <Text className="w-[20%] font-bold">{baseRate}</Text>
      <Text className="w-[20%] font-bold">{remitSend}</Text>
      <Text className="w-[20%] font-bold">{remitReceive}</Text>
