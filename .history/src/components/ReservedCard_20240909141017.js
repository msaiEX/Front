  ...
  return (
    <div onClick={onClick} className="cursor-pointer">
      <div className='flex gap-1 mt-2'>
        ...
        <Text
          name="사기팔기"
          className={`border-1 py-[0.1px] px-1 rounded-md ${getTransactionStyle(transaction_type)}`}
        >
          {getTransactionLabel(transaction_type)}
        </Text>
        <Text className='border-1 border-slate-300 text-slate-300 py-[0.1px] px-1 rounded-md'>예약</Text>
      </div>
      <div className='flex justify-between'>
        <div className='flex items-center gap-1 mt-2'>
          <Image 
