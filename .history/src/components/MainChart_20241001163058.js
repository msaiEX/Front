...
const MainChart = ({ id, changeState, consumData, todayData }) => {
  ...
  const [fade, setFade] = useState(false); // Fade 효과 상태 관리

  const handleClick = (range) => {
    setFade(true); // 클릭 시 fade 효과 시작
    setTimeout(() => {
      setTimeRange(range); // fade-in 후 값 변경
      setFade(false); // fade 완료 후 원래 상태로
    }, 300); // 300ms 후에 값이 변경되도록 설정 (애니메이션과 일치)
  };
        ...
        <button
          ...
          className={`px-4 py-2 rounded transition-opacity duration-300 ${
            timeRange === range
              ? "bg-white text-gray-800"
              : "bg-gray-200 text-gray-800"
          } ${fade ? "opacity-0" : "opacity-100"}`} // fade 효과 적용
