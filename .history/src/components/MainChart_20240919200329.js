...
const MainChart = ({ id, changeState, consumData, todayData }) => {
  ...
  const filterData = (range) => {
    ...
    switch (range) {
      case "1D":
        data = todayData; // 1D일 때는 todayData 사용
        break;
      case "1M":
        data = consumData.slice(-30);
        break;
  ...
  // 최고가, 최저가, 평균가 계산 (todayData의 경우 exchange_rate 사용)
  useEffect(() => {
    if (filteredData.length > 0) {
      let prices;
      if (timeRange === "1D") {
        prices = filteredData.map((data) => parseFloat(data.exchange_rate)); // todayData에서 exchange_rate 사용
      } else {
        prices = filteredData.flatMap((data) => data.price.map(Number)); // 다른 경우 price 배열 사용
      }
      ...
      <div>
        ...
        >
          ...
          {timeRange === "1D" ? (
            <Area
              type="monotone"
              dataKey="exchange_rate" // todayData에서 exchange_rate 사용
              stroke="#b6e9e5"
              fill="#b6e9e5"
            />
          ) : (
            <>
              <Area
                type="monotone"
                dataKey={(data) => data.price[0]} // price 배열의 첫 번째 값(최고가)
                stroke="#b6e9e5"
                fill="#b6e9e5"
              />
              <Area
