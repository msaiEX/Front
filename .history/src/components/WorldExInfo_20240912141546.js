...
const WorldExInfo = ({ currency, baseRate, remitSend, remitReceive, updownRate, isSelected }) => {
  ...
  return (
    <div className="w-full text-center flex mb-2 py-2 rounded-full" style={{boxShadow: "0 1px 5px rgba(0, 0, 0, 0.2)"}}>
      <div className="w-[35%] flex gap-1 justify-start items-center px-3">
      ...
      <Text className="w-[15%] font-bold py-2">{baseRate}</Text>
      <Text className="w-[15%] font-bold py-2">{remitSend}</Text>
      <Text className="w-[15%] font-bold py-2">{remitReceive}</Text>
