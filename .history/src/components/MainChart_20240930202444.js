...
//       ...item,
//       "매매기준율": item.baserate, // baserate를 매매기준율로 변환
//       "최저가-최고가": `${Math.min(...item.price)}-${Math.max(...item.price)}`, // price 배열을 최저가-최고가로 변환
//     }));
//   };
//   // 데이터 필터링 함수
...
const MainChart = ({ id, changeState, consumData, todayData }) => {
  ...
  const filterData = (range) => {
    ...
    // JPY이고 1Y일 때 내림차순으로 정렬
    if (changeState === "JPY" && range === "1Y") {
      data = data.slice().sort((a, b) => b.day.localeCompare(a.day));
    }

    return transformData(data); // 데이터를 변환한 후 반환
  };
  ...
  useEffect(() => {
    if (timeRange === "1D" && todayData.length > 0) {
      ...
      const min = Math.min(...exchangeRates);
      const avg =
        exchangeRates.reduce((acc, curr) => acc + curr, 0) / exchangeRates.length;
