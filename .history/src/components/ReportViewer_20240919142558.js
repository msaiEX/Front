    ...
    <div className="w-full px-5">
      ...
      <div className="flex justify-center bg-[#d6e1fd] my-4">
        ...
        <button
          ...
          className={`w-[25%] px-2 py-2  ${activeButton === 'month' ? 'bg-[#d6e1fd] text-blue-600' : 'bg-[#f1f3f8] text-slate-700'}`}
        >
          Month Report
        </button>
        <button
          onClick={() => handleReportChange('week')}
          className={`w-[25%] px-2 py-2  ${activeButton === 'week' ? 'bg-[#d6e1fd] text-blue-600' : 'bg-[#f1f3f8] text-slate-700'}`}
        >
          Week Report
        </button>
        <button
          onClick={() => handleReportChange('day')}
          className={`w-[25%] px-2 py-2  ${activeButton === 'day' ? 'bg-[#d6e1fd] text-blue-600' : 'bg-[#f1f3f8] text-slate-700'}`}
        >
          Day Report
        </button>
      </div>
      ...
      {/* 선택된 보고서 데이터를 보여줄 영역 */}
      <div name="1-2" className="bg-[#e7ffe7] h-[400px] rounded-xl mt-6 py-3 px-2">
      ...
      <div className="flex mt-6 gap-2">
        <a
          ...
          download // 다운로드 속성 추가
          className="px-4 py-2 bg-[#CBE2B5] text-blue-600 rounded"
        >
          반기상황
        </a>
        <a
