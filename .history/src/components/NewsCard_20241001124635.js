...
const NewsCard = ({ state, url, title, content, image_url, result }) => {
  ...
  return (
    <Link
      ...
      name="card-container"
      className='hover:transform hover:scale-95 hover:border hover:border-green-300 transition-all duration-300 ease-in-out'
      style={{
      ...
      <div name="text-box" className="flex flex-col items-center">
        ...
        <div className="w-full flex justify-end items-center pb-4 px-3">
          <Text
            fontSize={"xl"}
            as={"b"}
            className="absolute bottom-[20px] left-[110px] my-2"
            color={resultColor}
          >
            {result}
          </Text>
