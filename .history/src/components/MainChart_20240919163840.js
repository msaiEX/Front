...
const MainChart = ({ id, changeState, consumData, todayData }) => {
  ...
  const getYAxisDomain = () => {
    ...
    // changeState가 "JPY"일 경우 추가적인 범위 조정
    if (changeState === "JPY") {
      minValue = Math.max(minValue, 880); // 최소값을 990 이상으로 고정
    }
    
    // 내림차순으로 Y축을 설정 (최대값에서 최소값으로)
    return [Math.ceil(maxValue + 5), Math.floor(minValue - 5)];
  };
    ...
    <div>
      <div>
        ...
        >
          ...
          />
          <YAxis domain={getYAxisDomain()} reversed={true} />
