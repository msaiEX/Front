...
const NewsBar = ({ state, result, url, title, content, imageUrl }) => {
  ...
  return (
    <Link to={"/NewsPageDetail/" + encodeURIComponent(url)}>
      <div
        className="w-full flex justify-center py-2 px-10 mb-4 rounded-2xl bg-[#eff6f3] transition-all duration-500 ease-in-out opacity-0 animate-fadein hover:border hover:border-green-500"
