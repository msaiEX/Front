...
const FlowText = () => {
  ...
  useEffect(() => {
    // HTML 데이터 문자열로 예제
    const htmlString = `
      <div class="wrap-ticker-list">
        <div class="ticker-item up"><i class="icon"></i><span class="ticker-item-unit">USD</span><span class="ticker-item-amount">1,348.50</span><span class="ticker-item-point">3.80</span></div>
        <div class="ticker-item up"><i class="icon"></i><span class="ticker-item-unit">JPY</span><span class="ticker-item-amount">912.13</span><span class="ticker-item-point">4.10</span></div>
        <div class="ticker-item up"><i class="icon"></i><span class="ticker-item-unit">EUR</span><span class="ticker-item-amount">1,481.73</span><span class="ticker-item-point">6.19</span></div>
        <div class="ticker-item up"><i class="icon"></i><span class="ticker-item-unit">CNY</span><span class="ticker-item-amount">191.11</span><span class="ticker-item-point">0.75</span></div>
      </div>`;

    const parser = new DOMParser();
    const doc = parser.parseFromString(htmlString, 'text/html');

    const items = Array.from(doc.querySelectorAll('.ticker-item')).map(item => {
      const name = item.querySelector('.ticker-item-unit').textContent;
      const value = item.querySelector('.ticker-item-amount').textContent;
      const change = item.querySelector('.ticker-item-point').textContent;
