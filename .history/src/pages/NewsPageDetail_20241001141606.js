...
//               <Text className="mb-4" ㅁ>{consumData.news_analysis}</Text>
//             </div>
//           </>
//         ) : (
...
const NewsPageDetail = () => {
  ...
  const gptBoxRef = useRef(null); // gpt-box의 콘텐츠를 감싸는 ref

  const [gptBoxHeight, setGptBoxHeight] = useState("auto"); // 초기 높이를 auto로 설정
  ...
  return (
    <div className="w-[960px] flex flex-col py-1 px-10 mt-10">
      ...
      <div
        name="gpt-box" 
        className="bg-green-200 px-6 py-4 rounded-2xl mb-4 overflow-hidden"
        style={{
          maxHeight: gptBoxHeight,
          transition: 'max-height 0.5s ease' // 애니메이션 적용
        }}
      >
        <div ref={gptBoxRef}>
          {gpttext === true ? (
