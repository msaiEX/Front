...
//                   </div>
                  ...
                  //   <div className="flex justify-between">
                  //     <Text className="text-slate-600">평균적용환율</Text>
                  //     <Text className="text-slate-600">{formatNumber((usdBuyTotal - usdSellTotal) / usdNetAmount)}원</Text>
                  //   </div>
                  ...
                  //   <div className="flex justify-between">
                  //     <Text className="text-xl font-medium">지금 팔면</Text>
                  //     <Text className="text-xl font-medium">{formatNumber(1331.2 * usdNetAmount)}원</Text>
                  //   </div>

                  //   <div className="flex justify-between">
                  //     <Text className="text-slate-600">예상적용환율</Text>
                  //     <Text className="text-slate-600">1331.2</Text>
                  //   </div>

                  //   <div className="flex justify-between">
                  //     <Text>예상수익금</Text>
                  //     <Text>{formatNumber((usdBuyTotal - usdSellTotal) - (1331.2 * usdNetAmount))}원</Text>
                  //   </div>

                  //   <div className="flex justify-between">
                  //     <Text>예상수익률</Text>
                  //     <Text>{((usdBuyTotal - usdSellTotal) / (1331.2 * usdNetAmount)).toFixed(2)}%</Text>
                  //   </div>
...
//         // 통화별 합산 처리
//         let buyTotalUSD = 0, sellTotalUSD = 0, netAmountUSD = 0;
//         let buyTotalJPY = 0, sellTotalJPY = 0, netAmountJPY = 0;
//         let buyTotalCNY = 0, sellTotalCNY = 0, netAmountCNY = 0;

//         completedTransactions.forEach((transaction) => {
//           const { currency_code, transaction_type, withdrawal_amount, deposit_amount } = transaction;
...
//                         ></Image>
//                         <Text className="text-xl font-medium">USD</Text>
//                         <Text className="text-xl font-medium">{formatNumber(usdNetAmount)}</Text>
//                       </div>
//                     </div>
...
//                         ></Image>
//                         <Text className="text-xl font-medium">JPY</Text>
//                         <Text className="text-xl font-medium">{formatNumber(jpyNetAmount)}</Text>
//                       </div>
//                     </div>
...
//                         <Image
//                           className="w-[24px] h-[24px]"
//                           src="/image/cny_flag.png"
//                         ></Image>
//                         <Text className="text-xl font-medium">CNY</Text>
//                         <Text className="text-xl font-medium">{formatNumber(cnyNetAmount)}</Text>
//                       </div>
//                     </div>

//                     <div className="flex justify-between">
//                       <Text className="text-xl font-medium">환산금액 (USD)</Text>
//                       <Text className="text-xl font-medium">{formatNumber(usdBuyTotal - usdSellTotal)}원</Text>
//                     </div>

//                     <div className="flex justify-between">
//                       <Text className="text-slate-600">평균적용환율 (USD)</Text>
//                       <Text className="text-slate-600">{formatNumber((usdBuyTotal - usdSellTotal) / usdNetAmount)}원</Text>
//                     </div>
...
//                     {/* JPY 추가 로직 */}
//                     <div className="flex justify-between">
//                       <Text className="text-xl font-medium">지금 팔면 (JPY)</Text>
//                       <Text className="text-xl font-medium">{formatNumber(1331.2 * jpyNetAmount)}원</Text>
//                     </div>

//                     <div className="flex justify-between">
//                       <Text className="text-slate-600">예상적용환율 (JPY)</Text>
//                       <Text className="text-slate-600">1331.2</Text>
//                     </div>

//                     <div className="flex justify-between">
//                       <Text>예상수익금 (JPY)</Text>
//                       <Text>{formatNumber((jpyBuyTotal - jpySellTotal) - (1331.2 * jpyNetAmount))}원</Text>
//                     </div>

//                     <div className="flex justify-between">
//                       <Text>예상수익률 (JPY)</Text>
//                       <Text>{((jpyBuyTotal - jpySellTotal) / (1331.2 * jpyNetAmount)).toFixed(2)}%</Text>
//                     </div>

//                     {/* CNY 추가 로직 */}
//                     <div className="flex justify-between">
//                       <Text className="text-xl font-medium">지금 팔면 (CNY)</Text>
//                       <Text className="text-xl font-medium">{formatNumber(1331.2 * cnyNetAmount)}원</Text>
//                     </div>

//                     <div className="flex justify-between">
//                       <Text className="text-slate-600">예상적용환율 (CNY)</Text>
//                       <Text className="text-slate-600">1331.2</Text>
//                     </div>

//                     <div className="flex justify-between">
//                       <Text>예상수익금 (CNY)</Text>
//                       <Text>{formatNumber((cnyBuyTotal - cnySellTotal) - (1331.2 * cnyNetAmount))}원</Text>
//                     </div>

//                     <div className="flex justify-between">
//                       <Text>예상수익률 (CNY)</Text>
//                       <Text>{((cnyBuyTotal - cnySellTotal) / (1331.2 * cnyNetAmount)).toFixed(2)}%</Text>
//                     </div>
  ...
  useEffect(() => {
    ...
    const fetchAccountsData = async () => {
      try {
        ...
        console.log("transaction", response.data.transactions)
        console.log("accountData", response.data.accounts[1].jpy)
        // transaction_id에 따라 내림차순으로 정렬
        const sortedTransactions = [...completedTransactions].sort(
          (a, b) => b.transaction_id - a.transaction_id
        );
            ...
            <TabPanels>
              <TabPanel>
                ...
                >
                  ...
                  {/* "USD", "JPY", "CNY" 거래내역 요약 표시 */}
                  <div className="mt-4">
                    <Text className="font-bold text-lg">
                      통화별 거래 내역 요약
                    </Text>
                    <Text>USD 총 구매 금액 (buy): {formatNumber(usdBuyTotal)} USD</Text>
                    <Text>USD 총 판매 금액 (sell): {formatNumber(usdSellTotal)} USD</Text>
                    <Text>JPY 총 구매 금액 (buy): {formatNumber(jpyBuyTotal)} JPY</Text>
                    <Text>JPY 총 판매 금액 (sell): {formatNumber(jpySellTotal)} JPY</Text>
                    <Text>CNY 총 구매 금액 (buy): {formatNumber(cnyBuyTotal)} CNY</Text>
                    <Text>CNY 총 판매 금액 (sell): {formatNumber(cnySellTotal)} CNY</Text>
                  </div>
                </div>
              </TabPanel>
              ...
              <TabPanel>
                ...
                <div className="flex flex-col items-start ml-3">
                  ...
                  <div className="bg-white w-full rounded-2xl px-8 py-6 mt-4">
                    ...
                    {/* 외화대가외화금액 (USD, JPY, CNY) */}
                    <CurrencyAsset
                      currencyCode="USD"
                      imageSrc="/image/usd_flag.png"
                      amount={formatNumber(accountsData[1].usd)}
                      exchangeAmount={formatNumber(accountsData[1].usd)}
                      convertedAmount={formatNumber(usdBuyTotal - usdSellTotal)}
                      averageExchangeRate={formatNumber((usdBuyTotal - usdSellTotal) / usdNetAmount)}
                      currentSellAmount={formatNumber(1331.2 * usdNetAmount)}
                      expectedExchangeRate="1331.2"
                      expectedProfit={formatNumber((usdBuyTotal - usdSellTotal) - (1331.2 * usdNetAmount))}
                      expectedProfitRate={((usdBuyTotal - usdSellTotal) / (1331.2 * usdNetAmount)).toFixed(2)}
                    />

                    {/* <CurrencyAsset
                      currencyCode="JPY"
                      imageSrc="/image/jpy_flag.png"
                      amount={formatNumber(accountsData[2].jpy)} // accountsData[2]가 JPY 데이터를 담고 있다고 가정
                      exchangeAmount={formatNumber(jpyNetAmount)}
                      convertedAmount={formatNumber(jpyBuyTotal - jpySellTotal)}
                      averageExchangeRate={formatNumber((jpyBuyTotal - jpySellTotal) / jpyNetAmount)}
                      currentSellAmount={formatNumber(1331.2 * jpyNetAmount)}
                      expectedExchangeRate="1331.2"
                      expectedProfit={formatNumber((jpyBuyTotal - jpySellTotal) - (1331.2 * jpyNetAmount))}
                      expectedProfitRate={((jpyBuyTotal - jpySellTotal) / (1331.2 * jpyNetAmount)).toFixed(2)}
                    />

                    <CurrencyAsset
                      currencyCode="CNY"
                      imageSrc="/image/cny_flag.png"
                      amount={formatNumber(accountsData[3].cny)} // accountsData[3]가 CNY 데이터를 담고 있다고 가정
                      exchangeAmount={formatNumber(cnyNetAmount)}
                      convertedAmount={formatNumber(cnyBuyTotal - cnySellTotal)}
                      averageExchangeRate={formatNumber((cnyBuyTotal - cnySellTotal) / cnyNetAmount)}
                      currentSellAmount={formatNumber(1331.2 * cnyNetAmount)}
                      expectedExchangeRate="1331.2"
                      expectedProfit={formatNumber((cnyBuyTotal - cnySellTotal) - (1331.2 * cnyNetAmount))}
                      expectedProfitRate={((cnyBuyTotal - cnySellTotal) / (1331.2 * cnyNetAmount)).toFixed(2)}
                    /> */}
