            ...
            {selectedCurrencies.map((currencyCode, index) => {
              ...
              return (
                ...
                <ChartCard
                  ...
                  index={index + 1}
                  name={currencyCode}
                  imageUrl="/image/favorite_box.png" // 기본 이미지 경로
                  currentPrice="N/A" // 데이터가 없으므로 기본값 설정
                  increase="0" // 변동률 기본값 설정
                  date="N/A" // 날짜 기본값 설정
                  chartData={[]} // 데이터가 없으므로 빈 배열로 설정
                />
              );
            })}
          </div>
          ...
          <div className="flex justify-center mt-3">
            {filteredData.length > 10 && (
              <button onClick={() => setShowAll(!showAll)}>
                {showAll ? (
                  <div className="w-[120px] flex justify-center items-center rounded-full p-1 bg-green-500 text-white hover:bg-white hover:text-green-500 border border-green-500 transition duration-300">
                    접기 <TiArrowSortedUp />
                  </div>
                ) : (
                  <div className="w-[120px] flex justify-center items-center rounded-full p-1 bg-green-500 text-white hover:bg-white hover:text-green-500 border border-green-500 transition duration-300">
