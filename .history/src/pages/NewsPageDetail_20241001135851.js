...
const NewsPageDetail = () => {
  ...
  const [gptBoxHeight, setGptBoxHeight] = useState("0px");
  ...
  // gpttext 상태 변화에 따라 gptBoxHeight 업데이트
  useEffect(() => {
    if (gptBoxRef.current) {
      if (gpttext) {
  ...
  // fulltext 상태 변화에 따라 contentBoxHeight 업데이트
  useEffect(() => {
    if (contentBoxRef.current) {
      if (fulltext) {
        setContentBoxHeight(`${contentBoxRef.current.scrollHeight}px`);
      } else {
        setContentBoxHeight("0px");
      }
    }
  }, [fulltext, paragraphs]);
