  ...
  useEffect(() => {
    ...
    const fetchAccountsData = async () => {
      try {
        ...
        // 거래 내역 중 "USD", "JPY", "CNY" 각각의 buy와 sell 합산
        ...
        completedTransactions.forEach((transaction) => {
          if (transaction.currency_code === "USD") {
            if (transaction.transaction_type === "buy") {
              ...
              netAmountUSD += transaction.deposit_amount; // buy이면 deposit_amount를 더함
            } else if (transaction.transaction_type === "sell") {
              sellTotalUSD += transaction.deposit_amount;
              netAmountUSD -= transaction.withdrawal_amount; // sell이면 withdrawal_amount를 뺌
            }
