...
const MyPage = () => {
  ...
  // 통화별 총 구매 금액, 판매 금액, 순수 금액을 저장할 state
  ...
  useEffect(() => {
    const fetchAccountsData = async () => {
      try {
        ...
        setCurrencyData(newCurrencyData); // 업데이트된 통화 데이터를 state에 저장
      } catch (error) {
        console.error("Error fetching account data:", error);
      } finally {
        setIsLoading(true);
  ...
  return (
    <div className="w-[1140px] h-[calc(100vh-60px)] flex flex-col py-10 px-10">
      {/* USD 관련 데이터 */}
      <div className="mt-4">
        <Text className="font-bold text-lg">USD 거래 내역 요약</Text>
        <Text>총 구매 금액 (buy): {formatNumber(currencyData.USD.buyTotal)} USD</Text>
        <Text>총 판매 금액 (sell): {formatNumber(currencyData.USD.sellTotal)} USD</Text>
        <Text>Net Amount: {formatNumber(currencyData.USD.netAmount)} USD</Text>
      </div>
