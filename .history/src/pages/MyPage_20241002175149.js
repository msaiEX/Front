...
const MyPage = () => {
  ...
  const [currencyData, setCurrencyData] = useState({
    USD: { buyTotal: 0, sellTotal: 0, netAmount: 0 },
    JPY: { buyTotal: 0, sellTotal: 0, netAmount: 0 },
    CNY: { buyTotal: 0, sellTotal: 0, netAmount: 0 },
    EUR: { buyTotal: 0, sellTotal: 0, netAmount: 0 },
  });
  ...
  useEffect(() => {
    const fetchAccountsData = async () => {
      try {
        ...
        // 새로운 currencyData 객체를 생성하여 업데이트
        ...
        completedTransactions.forEach((transaction) => {
          ...
          if (newCurrencyData[currency_code]) {
            if (transaction_type === "buy") {
              newCurrencyData[currency_code].buyTotal += withdrawal_amount;
              newCurrencyData[currency_code].netAmount += deposit_amount;
            } else if (transaction_type === "sell") {
              newCurrencyData[currency_code].sellTotal += deposit_amount;
              newCurrencyData[currency_code].netAmount -= withdrawal_amount;
            }
          }
        });
  ...
  let currentBalance = initialBalance; // 거래 내역마다 잔액을 갱신할 변수
              ...
              <TabPanel>
                ...
                <div className="flex flex-col items-start ml-3">
                  ...
                  <div className="bg-white w-full rounded-2xl px-8 py-6 mt-4">
                    ...
                    <div className="flex justify-between">
                      <Text className="text-xl font-medium">환산금액</Text>
                      <Text className="text-xl font-medium">{formatNumber(currencyData.USD.buyTotal - currencyData.USD.sellTotal)}원</Text>
                    </div>

                    <div className="flex justify-between">
                      <Text className="text-slate-600">평균적용환율</Text>
                      <Text className="text-slate-600">{formatNumber((currencyData.USD.buyTotal - currencyData.USD.sellTotal) / currencyData.USD.netAmount)}원</Text>
                    </div>
                    ...
                    <div className="flex justify-between">
                      <Text className="text-xl font-medium">지금 팔면</Text>
                      <Text className="text-xl font-medium">{formatNumber(1331.2 * currencyData.USD.netAmount)}원</Text>
                    </div>
                    ...
                    <div className="flex justify-between">
                      <Text>예상수익금</Text>
                      <Text>{formatNumber((currencyData.USD.buyTotal - currencyData.USD.sellTotal) - (1331.2 * currencyData.USD.netAmount))}원</Text>
                    </div>

                    <div className="flex justify-between">
                      <Text>예상수익률</Text>
                      <Text>{((currencyData.USD.buyTotal - currencyData.USD.sellTotal) / (1331.2 * currencyData.USD.netAmount)).toFixed(2)}%</Text>
                    </div>

                    {/* 로직 추가 */}
                    <div className="mt-4">
                      <Text className="font-bold text-lg">JPY 거래 내역 요약</Text>
                      <Text>총 구매 금액 (buy): {formatNumber(currencyData.JPY.buyTotal)} JPY</Text>
                      <Text>총 판매 금액 (sell): {formatNumber(currencyData.JPY.sellTotal)} JPY</Text>
                      <Text>Net Amount: {formatNumber(currencyData.JPY.netAmount)} JPY</Text>
                    </div>

                    <div className="mt-4">
                      <Text className="font-bold text-lg">CNY 거래 내역 요약</Text>
                      <Text>총 구매 금액 (buy): {formatNumber(currencyData.CNY.buyTotal)} CNY</Text>
                      <Text>총 판매 금액 (sell): {formatNumber(currencyData.CNY.sellTotal)} CNY</Text>
                      <Text>Net Amount: {formatNumber(currencyData.CNY.netAmount)} CNY</Text>
                    </div>

                    <div className="mt-4">
                      <Text className="font-bold text-lg">EUR 거래 내역 요약</Text>
                      <Text>총 구매 금액 (buy): {formatNumber(currencyData.EUR.buyTotal)} EUR</Text>
                      <Text>총 판매 금액 (sell): {formatNumber(currencyData.EUR.sellTotal)} EUR</Text>
                      <Text>Net Amount: {formatNumber(currencyData.EUR.netAmount)} EUR</Text>
                    </div>
                  </div>
                </div>
              </TabPanel>
