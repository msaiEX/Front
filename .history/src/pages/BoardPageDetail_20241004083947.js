...
const BoardPageDetail = () => {
  const [boards, setBoards] = useState([
    { name: "김하나", content: "요즘 원달러 너무 떨어지네요...", image: "/image/user2.png" },
    { name: "박하나", content: "최근 악재가 많아서 추매하지면 안돼요!", image: "/image/user3.png" }
  ]);
    ...
    <div className="w-[960px] flex py-1 px-10 mt-10">
      ...
      <div name="container" className='w-full px-2 bg-white rounded-lg' style={{boxShadow: "0 2px 11px rgba(0, 0, 0, 0.1)"}}>
        {boards.map((board, index) => (
          <NewBoard key={index} name={board.name} boardtext={board.content} boardImage={board.image} />
        ))}
