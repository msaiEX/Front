    ...
    const fetchAccountsData = async () => {
      try {
        ...
        completedTransactions.forEach((transaction) => {
          if (transaction.currency_code === "USD") {
            if (transaction.transaction_type === "buy") {
              buyTotalUSD += transaction.withdrawal_amount;
              netAmountUSD += transaction.deposit_amount;
            } else if (transaction.transaction_type === "sell") {
              sellTotalUSD += transaction.deposit_amount;
              netAmountUSD -= transaction.withdrawal_amount;
            }
          } else if (transaction.currency_code === "JPY") {
            if (transaction.transaction_type === "buy") {
              buyTotalJPY += transaction.withdrawal_amount;
              netAmountJPY += transaction.deposit_amount;
            } else if (transaction.transaction_type === "sell") {
              sellTotalJPY += transaction.deposit_amount;
              netAmountJPY -= transaction.withdrawal_amount;
            }
          } else if (transaction.currency_code === "CNY") {
            if (transaction.transaction_type === "buy") {
              buyTotalCNY += transaction.withdrawal_amount;
              netAmountCNY += transaction.deposit_amount;
            } else if (transaction.transaction_type === "sell") {
              sellTotalCNY += transaction.deposit_amount;
              netAmountCNY -= transaction.withdrawal_amount;
            }
          }
        });
