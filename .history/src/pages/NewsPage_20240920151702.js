...
import { Text, Image, Popover, PopoverTrigger, PopoverContent, PopoverArrow, PopoverBody, PopoverCloseButton, PopoverHeader } from "@chakra-ui/react";
import { Swiper, SwiperSlide } from "swiper/react";
import { Pagination } from "swiper/modules";
import axios from "axios";
...
const NewsPage = () => {
  ...
  const [isLoading, setIsLoading] = useState(false);
  const [selected, setSelected] = useState(true);
  const [selectedCurrency, setSelectedCurrency] = useState("USD"); // 기본값을 USD로 설정
  ...
  // 기본적으로 선택된 통화에 따라 필터링된 데이터를 보여줌
  const filteredData = consumData.filter((data) => {
    return data.state === selectedCurrency;
  });
    ...
    <SwiperContainer>
      <div className="w-[1140px] h-[calc(100vh-80px)] flex flex-col py-10 px-10">
        ...
        <div className="flex items-center">
          ...
          <Image className="h-[32px] w-[32px]" src="./image/news2_imoji.png" />
        </div>
        <Text fontSize="xl">
          외환 전문 AI를 통해 글로벌 외환 소식을 쉽게 접해보세요.
        </Text>
        ...
        <div className="flex justify-center items-center gap-1">
          ...
          <Popover placement="top">
            ...
            <PopoverContent>
              ...
              <PopoverBody>
                <div className="flex gap-2">
                  {imageMapper.map((img, index) => (
                    <Image
                      key={index}
                      className="w-[50px] h-[50px] cursor-pointer"
                      src={img.image}
                      alt={img.name}
                      onClick={() => handleCurrencyChange(img.name)} // 이미지 클릭 시 통화 변경
                    />
                  ))}
                </div>
              </PopoverBody>
