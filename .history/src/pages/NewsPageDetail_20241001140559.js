...
//       {/* bg-[#EAF2EF] */}
//       <div name="gpt-box" 
//         className="bg-green-200 px-6 py-4 rounded-2xl mb-4"
//       >
//         {gpttext === true ? (
//           <>
...
//         {/* 일반 button 태그 사용 */}
//         <button
//           className={`px-4 py-2 ${currentNewsType === "KOREA" ? "bg-white border border-blue-700 text-blue-700" : "bg-[#f1f3f8] text-slate-700"} rounded`}
//           onClick={() => setCurrentNewsType("KOREA")}
//         >
//           한국 뉴스
//         </button>
//         <button
//           className={`px-4 py-2 ${currentNewsType === "LOCAL" ? "bg-white border border-blue-700 text-blue-700" : "bg-[#f1f3f8] text-slate-700"} rounded`}
...
const NewsPageDetail = () => {
  ...
  const gptBoxRef = useRef(null); // gpt-box의 실제 콘텐츠 높이를 저장할 참조
  const contentBoxRef = useRef(null); // content-box의 실제 콘텐츠 높이를 저장할 참조

  const [gptBoxHeight, setGptBoxHeight] = useState("100px");
  const [contentBoxHeight, setContentBoxHeight] = useState("200px");
  ...
  const handleGpttext = () => {
    setGptText(!gpttext);
    if (!gpttext) {
      setGptBoxHeight(`${gptBoxRef.current.scrollHeight}px`);
    } else {
      setGptBoxHeight("100px");
    }
  };
  ...
  const handleFulltext = () => {
    setFulltext(!fulltext);
    if (!fulltext) {
      setContentBoxHeight(`${contentBoxRef.current.scrollHeight}px`);
    } else {
      setContentBoxHeight("200px");
    }
  };
  ...
  return (
    <div className="w-[960px] flex flex-col py-1 px-10 mt-10">
      ...
      <div
        name="gpt-box"
        className="bg-green-200 px-6 py-4 rounded-2xl mb-4 overflow-hidden transition-all duration-500 ease-in-out"
        style={{ height: gptBoxHeight }}
      >
        <div ref={gptBoxRef}>
          {gpttext === true ? (
            <>
      ...
      <div
        name="content-box"
        className="flex flex-col items-center rounded-2xl px-10 py-4 bg-white overflow-hidden transition-all duration-500 ease-in-out"
        style={{ height: contentBoxHeight, boxShadow: "0 2px 5px rgba(0, 0, 0, 0.1)" }}
      >
        <div ref={contentBoxRef}>
          ...
          <div className="flex items-center">
            <Text name="show-fulltext" onClick={handleFulltext} className="cursor-pointer">
              자세히보기
            </Text>
            {fulltext === true ? <TiArrowSortedUp /> : <TiArrowSortedDown />}
          </div>
