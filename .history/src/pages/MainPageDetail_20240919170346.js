...
const MainPageDetail = () => {
  ...
  // 가격 API 요청
  const fetchInvestPrice = async () => {
    try {
      setIsPriceLoading(true);
      const response = await axios.get("/api/investPrice");
      setCurrentInvestPrice(response.data.investPrice);
      setOriginInvestPrice(response.data.investPrice);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  // 거래 내역 API 요청
  const fetchConsumData = async () => {
    try {
      const response = await axios.get("/api/consumData");
      setConsumData(response.data.
    ...
    const getUseHistory = async () => {
      try {
        ...
        if (filteredTodayData.length > 0) {
          ...
          setCurrentInvestPrice(remit_send - remit_send * 0.008);
          setOriginInvestPrice(remit_send)
          // setCurrentSellPrice(remit_receive);
          setCurrentSellPrice(remit_receive + remit_receive * 0.008);
